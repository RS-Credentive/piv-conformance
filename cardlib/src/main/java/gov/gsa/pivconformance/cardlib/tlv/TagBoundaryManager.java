...
public class TagBoundaryManager {
     ...
     */
    public int lengthDelta(String containerOid, BerTag tag, int bytesLength)
        ...
        if (tlRules == null) {
            String errStr = (String.format("Rules for %s, tag 0x%02x is null",
                    APDUConstants.containerOidToNameMap.get(containerOid), Hex.toHexString(tag.bytes)));
        ...
        if (rv != 0) {
            if (tlr.hasSoftUpperBound()) {
                String errStr = String.format("Container %s, Tag %s varies from SP 800-73-4 table by %d", containerOid,
