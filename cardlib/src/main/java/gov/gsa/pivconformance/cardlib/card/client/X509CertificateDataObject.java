...
public class X509CertificateDataObject extends PIVDataObject {
    ...
    public boolean decode() {
                        ...
                        for (BerTlv tlv2 : values2) {
                            if (tlv2.isPrimitive()) {
                                ...
                                if (Arrays.equals(tlv2.getTag().bytes, TagConstants.CERTINFO_TAG)) {
                                    ...
                                    if (certInfoBuf != null
                                            && Arrays.equals(certInfoBuf, TagConstants.COMPRESSED_TAG)) {
                                        m_compressed = true;
                                    }
                                    s_logger.debug(
                                            "Cert buffer is " + ((m_compressed) ? "compressed" : "uncompressed"));
                                }
        ...
        try {
            ...
            m_x509ArtifactCache.saveObject("x509-artifacts", APDUConstants.getFileNameForOid(getOID()) + ".cer",
                    m_cert.getEncoded());
