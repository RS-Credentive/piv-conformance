...
public class AtomHelper {
    ...
    public static PIVDataObject getDataObject(String oid) {
        ...
        // FileInputStream("G:\\GSA\\GSA_GIT\\piv-conformance-pkix-11\\tools\\85b-swing-gui\\85b-swing-gui-201907010300\\"
        // + oid + ".bin");
        //
        // ) {
        //
        // long fileSize = new
        // File("G:\\GSA\\GSA_GIT\\piv-conformance-pkix-11\\tools\\85b-swing-gui\\85b-swing-gui-201907010300\\"
        // + oid + ".bin").length();
        //
        ...
        //
        // if (o.decode() != true) {
        // ConformanceTestException e = new ConformanceTestException("Failed to decode
        // object for OID " + oid);
        // fail(e);
        // }
        ...
        if (css == null) {
            ConformanceTestException e = new ConformanceTestException("Singleton is null");
            fail(e);
        }
        ...
        case PIV_OK:
            break;
        default:
            String errMsg = "Failed to retrieve data object for OID " + APDUConstants.oidNameMap.get(oid)
                    + " container";
            s_logger.error(errMsg);
            return null;
        }
        ...
        if ((o instanceof SignedPIVDataObject) && ((SignedPIVDataObject) o).getCertCount() > 1) {
            ConformanceTestException e = new ConformanceTestException(
                    "More than one cert found in " + APDUConstants.oidNameMap.get(oid) + " container");
            fail(e);
        }
    ...
    /**
     * Get a certificate from a container specified by oid. For the CHUID and
     * Security Object containers the certificate will be the CHUID content signer
    ...
    public static X509Certificate getCertificateForContainer(PIVDataObject pivDataObject) {
        ...
        if (pivDataObject instanceof CardHolderUniqueIdentifier || pivDataObject instanceof CardHolderBiometricData
    ...
    public static boolean isDataObjectPresent(String oid, boolean authenticate) {
        ...
        // Check that CardSettingsSingleton
        if (css == null) {
            s_logger.error("isDataObjectPresent({},{}) called, but couldn't get an instance of CardSettingsSingleton",
                    oid, authenticate);
            return false;
        }

        if (authenticate && css.getLastLoginStatus() == LOGIN_STATUS.LOGIN_FAIL) {
            s_logger.error("isDataObjectPresent({},{}) called, but login has already been attempted unsuccessfully",
                    oid, authenticate);
            return false;
        }
        ...
        if (authenticate) {
            try {
                ...
                s_logger.error("isDataObjectPresent({},{}) called, but login failed with a ConformanceTestException",
                        oid, authenticate, e);
                return false;
            }
        }
        ...
        if (o == null) {
            s_logger.error("isDataObjectPresent() failed to allocate a PIVDataObject for {} {} authentication.", oid,
                    authenticate ? "with" : "without");
            return false;
        }
        ...
        if (result != MiddlewareStatus.PIV_OK) {
            s_logger.error("isDataObjectPresent() failed to get data for {} {} authentication: {}.", oid,
                    authenticate ? "with" : "without", result);
            return false;
        }

        // if we got here, the object is present and has now been cached. others can
        // attempt a decode and complain if that fails
        return true;
    }
}
