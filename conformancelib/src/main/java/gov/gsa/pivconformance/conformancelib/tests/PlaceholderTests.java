...
public class PlaceholderTests {
     ...
     * Implementations of atoms waiting to be placed into their appropriate test
     * classes |
     * +----------------------------------------------------------------------------
     * -------+
     */
    ...
    // CCT parameter Type 2 model (one parameter, which could be a comma-separated
    // list, multi-select [OR])
    @ParameterizedTest(name = "{index} => oid = {0}")
    ...
    // If it's a cat, it must be:sleepy, if it's a dog, it must be:hungry, if it's
    // an elephant:sad
    // Model for CCT test parameter type 3 (comma-separated list of parameters, with
    // each parameter being a name:value pair)
    @ParameterizedTest(name = "{index} => oid = {0}")
    // @MethodSource("placeholderTestProviderType2")
    @DisplayName("PlaceholderTestParamType1Model.2 Test")
    ...
    /*
     * +-------------------------- Concrete Test Atoms
     * ------------------------------------+ | Implementations of atoms waiting to
     * be placed into their appropriate test classes |
     * +----------------------------------------------------------------------------
     * -------+
     */
