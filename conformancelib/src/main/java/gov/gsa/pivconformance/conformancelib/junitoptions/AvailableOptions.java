...
class AvailableOptions {
        ...
        @Option(names = { "--scan-class-path",
                        "--scan-classpath" }, split = ";|:", paramLabel = "PATH", arity = "0..1", description = "Scan all directories on the classpath or explicit classpath roots. " //
                                        ...
                                        + "entries supplied via -" + CP_OPTION
        ...
        // Implementation note: the @Option annotation is on a setter method to allow
        // validation.
         ...
         * A {@code ParameterException} is thrown if the same key is specified multiple
         * times on the command line.
         *
         * @param map the key-value pairs to add
         * @throws picocli.CommandLine.ParameterException if the map already contains
         *                                                this key
        ...
        private void validateUnique(String key, String newValue) {
                ...
                if (existing != null && !existing.equals(newValue)) {
                        throw new ParameterException(spec.commandLine(), String.format(
                                        "Duplicate key '%s' for values '%s' and '%s'.", key, existing, newValue));
                }
        }
        ...
        CommandLineOptions toCommandLineOptions(ParseResult parseResult) {
                ...
                result.setAdditionalClasspathEntries(
                                merge(this.additionalClasspathEntries, this.additionalClasspathEntries2));
                ...
                result.setSelectedClasspathEntries(
                                merge(this.selectedClasspathEntries, this.selectedClasspathEntries2));
                ...
                // Filters
                result.setIncludedClassNamePatterns(
                                merge(this.includeClassNamePatterns, this.includeClassNamePatterns2));
                result.setExcludedClassNamePatterns(
                                merge(this.excludeClassNamePatterns, this.excludeClassNamePatterns2));
