...
public class TestStepModel {
    ...
    public void retrieveForId(int testStepId, int testId) {
        ...
        String query = "SELECT " + "  TestStepId, Description, Class, Method, NumParameters, Status " + "FROM "
                + "  TestSteps " + "LEFT OUTER JOIN TestsToSteps ON " + "  TestSteps.Id = TestsToSteps.TestStepId "
        ...
        String parametersQuery = "SELECT " + "  Id, TestStepId, TestId, Value " + "FROM " + "  TestStepParameters "
                + "WHERE " + "  TestStepParameters.TestStepId = ? " + "ORDER BY " + "  TestStepParameters.ParamOrder";
        try {
            ...
            if (!rs.next()) {
                s_logger.error("Database not configured properly: no test case for id {}, step id {}", testId,
                        testStepId);
            ...
            // s_logger.debug("Test step {} has {} parameters", this.getTestDescription(),
            // nParameters);
