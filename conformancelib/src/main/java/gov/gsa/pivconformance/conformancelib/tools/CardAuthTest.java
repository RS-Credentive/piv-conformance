...
public class CardAuthTest {
    ...
    static {
        ...
        s_options.addOption("l", "login", false,
                "Log in to PIV applet using PIV application PIN prior to attempting dump");
    ...
    public static void main(String[] args) {
        ...
        if (cmd.hasOption("listOids")) {
            for (HashMap.Entry<String, Integer> entry : APDUConstants.oidToContainerIdMap.entrySet()) {
                ...
                System.out.println(
                        oid + ": " + names.get(oid) + "(Container ID 0x" + Integer.toHexString(entry.getValue()) + ")");
            }
            System.exit(0);
        }
        ...
        if (cmd.hasOption("listReaders")) {
            ...
            for (CardTerminal reader : readers) {
                ...
                System.out.println("[" + readerNum + "]: " + reader.getName() + " ("
                        + (cardInserted ? "card present" : "card not present") + ")");
            }
        ...
        String containerOid : cmd.getArgList()) {
            ...
            if (containerCert.getPublicKey() instanceof RSAPublicKey) {
                ...
                if (cmd.hasOption("printAlgs")) {
                    s_logger.debug("Certificate from container {} has a public key with algorithm {}", containerOid,
                            getPublicKeyAlgFromCertificate(containerCert));
                    s_logger.debug("JCE key algorithm: {}", jceKeyAlg);
                    s_logger.debug("Certificate from container {} was signed with algorithm {}", containerOid,
                            getSignatureAlgFromCertificate(containerCert));
                }
                ...
                byte[] paddedChallenge = GeneralAuthenticateHelper.preparePKCS1Challenge(challenge, digestOid,
                        modulusLen);
                ...
                // XXX *** for now, the digest we'll use in the challenge block is always sha256
                // for RSA. We likely need to change that.
                boolean verified = GeneralAuthenticateHelper.verifyResponseSignature(containerCert.getSigAlgName(),
                        pubKey, cr, challenge);
