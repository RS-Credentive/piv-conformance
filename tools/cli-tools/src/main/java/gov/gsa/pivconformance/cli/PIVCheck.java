...
public class PIVCheck {
        ...
        public static boolean TestCard(CardHandle c) {
                if (c.isValid()) {
                        ...
                        if (result == MiddlewareStatus.PIV_OK) {
                                ...
                                byte[] coexistentTagAllocationAuthority = cardAppProperties
                                                .getCoexistentTagAllocationAuthority();

                                if (appID != null)
                                        s_logger.info("Application identifier of application: {}",
                                                        Hex.encodeHexString(appID));
                                ...
                                if (cryptoAlgs != null) {
                                        for (byte[] b : cryptoAlgs) {
                                                ...
                                                s_logger.info("Algorithm ID: {} Algorithm Description: {}",
                                                                Hex.encodeHexString(b), TagConstants.algMAP.get(b));
                                        }
                                }
                                ...
                                for (BerTlv tlv : values) {
                                        if (tlv.isPrimitive()) {
                                                s_logger.info("PCAP Tag {}: {}",
                                                                Hex.encodeHexString(tlv.getTag().bytes),
                                                                Hex.encodeHexString(tlv.getBytesValue()));
                                        } else {
                                                s_logger.info("PCAP object: {}",
                                                                Hex.encodeHexString(tlv.getTag().bytes));
                                        }
                                }
                                ...
                                //
                                // if(result == MiddlewareStatus.PIV_OK)
                                // s_logger.info("Number of containers present on the card: {}",
                                // dataList.size());
                                //
                                //
                                // PIVDataObject keyDataObject =
                                // PIVDataObjectFactory.createDataObjectForOid("1.2.3");
                                // result = piv.pivGenerateKeyPair(c, APDUConstants.PIV_AUTHENTICATION_KEY,
                                // APDUConstants.CRYPTO_MECHANISM_RSA, keyDataObject);
                                //
                                // if(result == MiddlewareStatus.PIV_OK)
                                ...
                                for (String containerOID : APDUConstants.MandatoryContainers()) {
                                        ...
                                        if (containerOID.equals(APDUConstants.CARD_CAPABILITY_CONTAINER_OID)) {
                                                ...
                                                s_logger.info("Card Identifier: {}",
                                                                Hex.encodeHexString(
                                                                                ((CardCapabilityContainer) dataObject)
                                                                                                .getCardIdentifier()));
                                                s_logger.info("Capability Container Version Number: {}", Hex
                                                                .encodeHexString(((CardCapabilityContainer) dataObject)
                                                                                .getCapabilityContainerVersionNumber()));
                                                s_logger.info("Capability Grammar Version Number: {}", Hex
                                                                .encodeHexString(((CardCapabilityContainer) dataObject)
                                                                                .getCapabilityGrammarVersionNumber()));

                                                List<byte[]> appCardURLList = ((CardCapabilityContainer) dataObject)
                                                                .getAppCardURL();
                                                ...
                                                s_logger.info("Registered Data Model number: {}", Hex
                                                                .encodeHexString(((CardCapabilityContainer) dataObject)
                                                                                .getRegisteredDataModelNumber()));
                                                s_logger.info("Access Control Rule Table: {}", Hex
                                                                .encodeHexString(((CardCapabilityContainer) dataObject)
                                                                                .getAccessControlRuleTable()));
                                                ...
                                                s_logger.info("Status Tuples Tag Present: {}",
                                                                ((CardCapabilityContainer) dataObject)
                                                                                .getStatusTuples());
                                                s_logger.info("Next CCC Tag Present: {}",
                                                                ((CardCapabilityContainer) dataObject).getNextCCC());

                                                if (((CardCapabilityContainer) dataObject)
                                                                .getExtendedApplicationCardURL() != null) {
                                                        ...
                                                        if (extendedAppCardURLList.size() > 0) {
                                                                ...
                                                                for (byte[] u2 : extendedAppCardURLList) {
                                                                        s_logger.info("     {}",
                                                                                        Hex.encodeHexString(u2));
                                                                }
                                                        }
                                                }

                                                if (((CardCapabilityContainer) dataObject)
                                                                .getSecurityObjectBuffer() != null)
                                                        s_logger.info("Security Object Buffer: {}", Hex.encodeHexString(
                                                                        ((CardCapabilityContainer) dataObject)
                                                                                        .getSecurityObjectBuffer()));
                                        ...
                                        if (containerOID.equals(APDUConstants.CARD_HOLDER_UNIQUE_IDENTIFIER_OID)) {
                                                ...
                                                while (it.hasNext()) {
                                                        ...
                                                        String serial = Hex.encodeHexString(
                                                                        sid.getSerialNumber().toByteArray());
                                                        String skid = "";
                                                        if (sid.getSubjectKeyIdentifier() != null)
                                                                skid = Hex.encodeHexString(
                                                                                sid.getSubjectKeyIdentifier());

                                                        if (sid.getSubjectKeyIdentifier() != null)
                                                                s_logger.info("Signer skid: {} ", skid);
                                                        else
                                                                s_logger.info("Signer Issuer: {}, Serial Number: {} ",
                                                                                issuer, serial);
                                        ...
                                        if (containerOID.equals(
                                                ...
                                                X509Certificate pibAuthCert = ((X509CertificateDataObject) dataObject)
                                                                .getCertificate();

                                                s_logger.info("PIV Auth Cert SubjectName: {}",
                                                                pibAuthCert.getSubjectDN().getName());
                                                s_logger.info("PIV Auth Cert SerialNumber: {}", Hex.encodeHexString(
                                                                pibAuthCert.getSerialNumber().toByteArray()));
                                                s_logger.info("PIV Auth Cert IssuerName: {}",
                                                                pibAuthCert.getSubjectDN().getName());
                                        }
                                        ...
                                        if (containerOID.equals(APDUConstants.SECURITY_OBJECT_OID)) {
                                                ...
                                                s_logger.info("RAW Mapping of DG to ContainerID value: {}",
                                                                Hex.encodeHexString(((SecurityObject) dataObject)
                                                                                .getMapping()));

                                                HashMap<Integer, String> idMap = ((SecurityObject) dataObject)
                                                                .getContainerIDList();
                                                ...
                                                s_logger.info("SecurityObject signatue valid: {}",
                                                                ((SecurityObject) dataObject)
                                                                                .verifySignature(signingCertificate));
                                        ...
                                        if (containerOID.equals(
                                                ...
                                                s_logger.info("Key Managment Cert SubjectName: {}",
                                                                pibAuthCert.getSubjectDN().getName());
                                                s_logger.info("Key Managment Cert SerialNumber: {}",
                                                                Hex.encodeHexString(pibAuthCert.getSerialNumber()
                                                                                .toByteArray()));
                                                s_logger.info("Key Managment Cert IssuerName: {}",
                                                                pibAuthCert.getSubjectDN().getName());
                                        }
                                        ...
                                        if (containerOID.equals(
                                                ...
                                                s_logger.info("Digital Signature Cert SubjectName: {}",
                                                                pibAuthCert.getSubjectDN().getName());
                                                s_logger.info("Digital Signature SerialNumber: {}", Hex.encodeHexString(
                                                                pibAuthCert.getSerialNumber().toByteArray()));
                                                s_logger.info("Digital Signature IssuerName: {}",
                                                                pibAuthCert.getSubjectDN().getName());
                                        }
                                        ...
                                        if (containerOID.equals(
                                                ...
                                                s_logger.info("Card Auth Cert SubjectName: {}",
                                                                pibAuthCert.getSubjectDN().getName());
                                                s_logger.info("Card Auth Cert SerialNumber: {}", Hex.encodeHexString(
                                                                pibAuthCert.getSerialNumber().toByteArray()));
                                                s_logger.info("Card Auth Cert IssuerName: {}",
                                                                pibAuthCert.getSubjectDN().getName());
                                        }
                                ...
                                if (result == MiddlewareStatus.PIV_OK) {
                                        ...
                                        if (decoded) {
                                                s_logger.info("Name: {}",
                                                                ((PrintedInformation) printedInformation).getName());
                                                s_logger.info("Employee Affiliation: {}",
                                                                ((PrintedInformation) printedInformation)
                                                                                .getEmployeeAffiliation());
                                                s_logger.info("Expiration date: {}",
                                                                ((PrintedInformation) printedInformation)
                                                                                .getExpirationDate());
                                                s_logger.info("Agency Card Serial Number: {}",
                                                                ((PrintedInformation) printedInformation)
                                                                                .getAgencyCardSerialNumber());
                                                s_logger.info("Issuer Identification: {}",
                                                                ((PrintedInformation) printedInformation)
                                                                                .getIssuerIdentification());
                                                if (((PrintedInformation) printedInformation)
                                                                .getOrganizationAffiliation1() != "")
                                                        s_logger.info("Name: {}",
                                                                        ((PrintedInformation) printedInformation)
                                                                                        .getOrganizationAffiliation1());
                                                if (((PrintedInformation) printedInformation)
                                                                .getOrganizationAffiliation2() != "")
                                                        s_logger.info("Name: {}",
                                                                        ((PrintedInformation) printedInformation)
                                                                                        .getOrganizationAffiliation2());
                                                s_logger.info("Error Detection Code Tag Present: {}",
                                ...
                                // result = piv.pivPutData(c, APDUConstants.PRINTED_INFORMATION_OID,
                                // printedInformation);
                                // if(result != MiddlewareStatus.PIV_OK) {
                                ...
                                if (result == MiddlewareStatus.PIV_OK) {
                                        ...
                                        s_logger.info("{} {}",
                                                        biometricInformationTemplatesGroupTemplate.getFriendlyName(),
                                                        decoded ? "decoded successfully" : "failed to decode");
                                ...
                                if (result == MiddlewareStatus.PIV_OK) {
                                        ...
                                        if (decoded) {
                                                ...
                                                s_logger.info("Content Signing Cert SerialNumber: {}",
                                                                Hex.encodeHexString(contentSigningCert.getSerialNumber()
                                                                                .toByteArray()));
                                                ...
                                                if (((SecureMessagingCertificateSigner) secureMessagingCertificateSigner)
                                                        ...
                                                        s_logger.info("Intermediate CVC: {}", Hex.encodeHexString(
        ...
        public static void main(String[] args) {
                ...
                for (CardTerminal t : terminals) {
                        ...
                        if (descriptor != null) {
                                s_logger.info("Descriptor for terminal {}: {}", terminalCount,
                                                Hex.encodeHexString(descriptor, false));
                        }
                        ...
                        s_logger.info("[{}] PIVMiddleware.pivConnect() returned {} for reader {}", terminalCount,
                                        result, t.getName());
