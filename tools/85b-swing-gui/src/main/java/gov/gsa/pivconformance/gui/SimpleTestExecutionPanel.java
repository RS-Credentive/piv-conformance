...
public class SimpleTestExecutionPanel extends JPanel {
    ...
    public SimpleTestExecutionPanel() {
        ...
        m_readerComboBox.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    JOptionPane msgBox = new JOptionPane("Unable to configure reader: "
                            + m_readerComboBox.getSelectedItem() + ".\n" + e1.getMessage(), JOptionPane.ERROR_MESSAGE);
        ...
        m_runButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    // Armen -- Added this snippet to refresh card information before each run,
                    // otherwise if cards are swapped and
                    // run tests button is click tests fail. Probably would be better to have an
                    // action that would detect
                    // a new card do this when new card is detected but not sure how to do that
                    // right now
                    int selected = m_readerComboBox.getSelectedIndex();
                    CardSettingsSingleton css = CardSettingsSingleton.getInstance();
                    css.reset();
                    ...
                    if (!CardUtils.setUpPivAppHandleInSingleton()) {
                        ...
                        JOptionPane msgBox = new JOptionPane("Error connecting to card in order to verify PIN.",
                                JOptionPane.ERROR_MESSAGE);
                        JDialog dialog = msgBox.createDialog(SimpleTestExecutionPanel.this, "Error");
                        dialog.setAlwaysOnTop(true);
                        dialog.setVisible(true);
                    ...
                    JOptionPane msgBox = new JOptionPane("Unable to connect to PIV in reader "
                            + m_readerComboBox.getSelectedItem() + ".\n" + e1.getMessage(), JOptionPane.ERROR_MESSAGE);
                    JDialog dialog = msgBox.createDialog(SimpleTestExecutionPanel.this, "Error");
                    dialog.setAlwaysOnTop(true);
                    dialog.setVisible(true);
                ...
                // first, check to make sure we have more than one retry remaining for
                // application PIN
                ...
                if (nRetries == -1) {
                    ...
                    JOptionPane msgBox = new JOptionPane(
                            "Unable to verify PIN prior to testing. Confirm that the card is not locked before proceeding.",
                            JOptionPane.ERROR_MESSAGE);
                    JDialog dialog = msgBox.createDialog(SimpleTestExecutionPanel.this, "Error");
                    dialog.setAlwaysOnTop(true);
                    dialog.setVisible(true);
                ...
                if (m_appPinField.getPassword().length < 6 || m_appPinField.getPassword().length > 8) {
                    JOptionPane msgBox = new JOptionPane("Application PIN must be between 6 and 8 digits.",
                            JOptionPane.ERROR_MESSAGE);
                    JDialog dialog = msgBox.createDialog(SimpleTestExecutionPanel.this, "Error");
                    dialog.setAlwaysOnTop(true);
                    dialog.setVisible(true);
                ...
                try {
                    if (!CardUtils.authenticateInSingleton(false)) {
                        ...
                        JOptionPane msgBox = new JOptionPane(
                                "Incorrect application PIN. " + nRetries + " tries remain.", JOptionPane.ERROR_MESSAGE);
                        JDialog dialog = msgBox.createDialog(SimpleTestExecutionPanel.this, "Error");
                        dialog.setAlwaysOnTop(true);
                        dialog.setVisible(true);
                        return;
                    ...
                    JOptionPane msgBox = new JOptionPane("Unable to authenticate to PIV in reader "
                            + m_readerComboBox.getSelectedItem() + ".\n" + e1.getMessage(), JOptionPane.ERROR_MESSAGE);
                    JDialog dialog = msgBox.createDialog(SimpleTestExecutionPanel.this, "Error");
                    dialog.setAlwaysOnTop(true);
                    dialog.setVisible(true);
        ...
        groupLayout.setHorizontalGroup(groupLayout.createParallelGroup(Alignment.LEADING).addGroup(Alignment.TRAILING,
                groupLayout.createSequentialGroup().addGroup(groupLayout.createParallelGroup(Alignment.LEADING)
                        .addGroup(groupLayout.createSequentialGroup().addGap(10)
                                .addGroup(groupLayout.createParallelGroup(Alignment.TRAILING, false)
                                        .addGroup(groupLayout.createSequentialGroup().addComponent(lblApplicationPin)
                                                .addGap(425).addComponent(m_appPinField, 0, 0, Short.MAX_VALUE))
                                        .addGroup(Alignment.LEADING, groupLayout.createSequentialGroup()
                                                .addComponent(lblCardReader).addGap(34).addComponent(m_readerComboBox,
                                                        GroupLayout.PREFERRED_SIZE, 507, GroupLayout.PREFERRED_SIZE))))
                        .addGroup(groupLayout.createSequentialGroup().addGap(10).addGroup(groupLayout
                                .createParallelGroup(Alignment.TRAILING)
                                .addGroup(groupLayout.createSequentialGroup().addComponent(lblReaderStatus).addGap(26)
                                        .addGroup(groupLayout.createParallelGroup(Alignment.TRAILING)
                                                .addComponent(btnOpenOtherDatabase).addComponent(m_readerStatusField,
                                                        GroupLayout.PREFERRED_SIZE, 507, GroupLayout.PREFERRED_SIZE)))
                                .addGroup(groupLayout.createSequentialGroup().addComponent(lblTestDatabase).addGap(25)
                                        .addComponent(m_databaseNameField, GroupLayout.PREFERRED_SIZE, 507,
                                                GroupLayout.PREFERRED_SIZE))))
                        .addGroup(groupLayout.createSequentialGroup().addGap(226).addComponent(lblImage,
                ...
                .addGroup(groupLayout.createSequentialGroup().addGap(106)
                        .addComponent(btnRefreshReaders, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE,
                                Short.MAX_VALUE)
                        .addPreferredGap(ComponentPlacement.RELATED).addComponent(m_runButton).addGap(317)));
        groupLayout.setVerticalGroup(groupLayout.createParallelGroup(Alignment.LEADING)
                .addGroup(groupLayout.createSequentialGroup().addGap(20).addComponent(lblImage).addGap(20)
                        .addGroup(groupLayout.createParallelGroup(Alignment.LEADING)
                                .addGroup(groupLayout.createSequentialGroup().addGap(3).addComponent(lblCardReader))
                                .addComponent(m_readerComboBox, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
                                        GroupLayout.PREFERRED_SIZE))
                        .addGroup(groupLayout.createParallelGroup(Alignment.LEADING)
                                .addGroup(
                                        groupLayout.createSequentialGroup().addGap(23).addComponent(lblApplicationPin))
                                .addGroup(groupLayout.createSequentialGroup().addGap(20).addComponent(m_appPinField,
                                        GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
                                        GroupLayout.PREFERRED_SIZE)))
                        .addGap(20)
                        .addGroup(groupLayout.createParallelGroup(Alignment.LEADING)
                                .addGroup(groupLayout.createSequentialGroup().addGap(3).addComponent(lblTestDatabase))
                                .addComponent(m_databaseNameField, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
                                        GroupLayout.PREFERRED_SIZE))
                        .addGap(27)
                        .addGroup(groupLayout.createParallelGroup(Alignment.TRAILING)
                                .addGroup(groupLayout.createSequentialGroup().addComponent(lblReaderStatus).addGap(18))
                                .addGroup(groupLayout.createSequentialGroup()
                                        .addComponent(m_readerStatusField, GroupLayout.PREFERRED_SIZE,
                                                GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(ComponentPlacement.UNRELATED)))
                        .addComponent(btnOpenOtherDatabase).addPreferredGap(ComponentPlacement.UNRELATED)
                        .addComponent(m_testProgressBar, GroupLayout.PREFERRED_SIZE, 23, GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(ComponentPlacement.UNRELATED)
                        .addGroup(groupLayout.createParallelGroup(Alignment.BASELINE).addComponent(btnRefreshReaders)
        ...
        try {
            if (reader.isCardPresent()) {
                try {
                    ...
                    JOptionPane msgBox = new JOptionPane("Unable to connect to reader: "
                            + m_readerComboBox.getSelectedItem() + ".\n" + e1.getMessage(), JOptionPane.ERROR_MESSAGE);
                    JDialog dialog = msgBox.createDialog(SimpleTestExecutionPanel.this, "Error");
                    dialog.setAlwaysOnTop(true);
                    dialog.setVisible(true);
            ...
            JOptionPane msgBox = new JOptionPane(
                    "Unable to connect to reader: " + m_readerComboBox.getSelectedItem() + ".\n" + e1.getMessage(),
                    JOptionPane.ERROR_MESSAGE);
            JDialog dialog = msgBox.createDialog(SimpleTestExecutionPanel.this, "Error");
            dialog.setAlwaysOnTop(true);
            dialog.setVisible(true);
